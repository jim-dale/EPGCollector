    ...
    public sealed class OutputFileXML
    {
        ...
        private static void processStationHeader(XmlWriter xmlWriter, TVStation tvStation)
        {
            ...
            if (string.IsNullOrWhiteSpace(RunParameters.Instance.ChannelLogoPath))
            {
                ...
                {
                    logoPath = RunParameters.DataDirectory + @"\Images\" + tvStation.Name + ".png";
                    if (!File.Exists(logoPath))
                        logoPath = null;
                }
                if (logoPath != null)
        ...
        private static void processEPGEntry(XmlWriter xmlWriter, string channelNumber, EPGEntry epgEntry)
        {
            ...
            {
                string[] categories = epgEntry.EventCategory.GetDescription(EventCategoryMode.Xmltv).Split(new char[] { ',' });
        ...
        {
            if (OptionEntry.IsDefined(OptionName.CreatePlexEpisodeNumTag) && (epgEntry.EventCategory != null && !epgEntry.EventCategory.GetDescription(EventCategoryMode.Xmltv).StartsWith("Movie")))
            {
