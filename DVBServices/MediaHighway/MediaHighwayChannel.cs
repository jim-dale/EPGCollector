...
namespace DVBServices
{
    ...
    public class MediaHighwayChannel : Channel
    {
        ...
        private EventCategorySpec getEventCategory(string title, string description, int categoryID)
        {
            if (categoryID == 0)
            {
                IEventCategory overrideCategory = getCustomCategory(title, description);
            ...
            if (category != null)
                return new EventCategorySpec(category.Description);

            if (OptionEntry.IsDefined(RunParameters.Instance.CurrentFrequency.AdvancedRunParamters.Options,
                OptionName.CustomCategoryOverride))
                return (null);

            IEventCategory customCategory = getCustomCategory(title, description);
