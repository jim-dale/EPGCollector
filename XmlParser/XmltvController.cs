...
namespace XmltvParser
{
    ...
    public sealed class XmltvController : ImportFileBase
    {
        ...
        public override string Process(string fileName, ImportFileSpec fileSpec)
        {
            ...
            Collection<TVStation> xmltvChannels = createEPGData(fileSpec.Language, fileSpec.Precedence, fileSpec.NoLookup, fileSpec.TimeZone);
        ...
        private Collection<TVStation> createEPGData(LanguageCode languageCode, DataPrecedence precedence, bool noLookup, string timeZone)
        {
            Collection<TVStation> stations = processChannels(languageCode);
            if (stations != null)
                processProgrammes(stations, languageCode, noLookup, timeZone);
        ...
        private void processProgrammes(Collection<TVStation> stations, LanguageCode languageCode, bool noLookup, string timeZone)
        {
            ...
            foreach (XmltvProgramme programme in XmltvProgramme.Programmes)
            {
                ...
                if (station != null)
                {
                    processProgramme(station, programme, languageCode, noLookup, timeZone);
                    created++;
                }
                else
                    noStation++;
        ...
        private void processProgramme(TVStation station, XmltvProgramme programme, LanguageCode languageCode, bool noLookup, string timeZone)
        {
            ...
            if (string.IsNullOrWhiteSpace(timeZone))
                ...
                epgEntry.StartTime = TimeZoneInfo.ConvertTimeBySystemTimeZoneId(programme.StartTime.Time.Value - programme.StartTime.Offset.Value, timeZone, TimeZoneInfo.Local.Id);
            ...
            if (programme.Categories != null && programme.Categories.Count != 0)
            {
                ...
                epgEntry.EventCategory = new EventCategorySpec(categoryString.ToString());
            }
