...
namespace Lookups
{
    internal class TVLookup
    {
        ...
        private bool CheckForTVSeries(EPGEntry epgEntry)
        {
            ...
            if (epgEntry.EventCategory != null && epgEntry.EventCategory.GetDescription(EventCategoryMode.Xmltv).ToLowerInvariant().Contains("istvseries"))
                return (true);
                        ...
                        if (!string.IsNullOrWhiteSpace(episode.Overview))
                        {
                            ...
                            else
                            {
                                if (seriesEntry.Overview.StartsWith(RunParameters.NoProgrammeDescription))
                                    seriesEntry.Overview = seriesEntry.Overview.Replace(RunParameters.NoProgrammeDescription, LookupController.ReplaceHtmlChars(episode.Overview));
                            }
                        }
